...
public class Guarder : Chessman
{
    ...
    private bool MoveLeft(int x, int y)
    {
        ...
        bool bRet = false;
        if (BoardDefine.SIDE_A == m_nSide) bRet = BoardDefine.SIDE_A_DEF.Guarder.InMyRange(nextX, nextY);
        else if (BoardDefine.SIDE_B == m_nSide) bRet = BoardDefine.SIDE_B_DEF.Guarder.InMyRange(nextX, nextY);
